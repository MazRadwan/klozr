...
import { Search, X, DollarSign, Plus, Loader2 } from 'lucide-react';
...
export function DealPicker({ contactId, currentDeals, onDealsUpdate }: DealPickerProps) {
      ...
      searchTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            // Filter out deals already linked to this contact
  ...
  const handleAddDeal = async (deal: DealSearchResult) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error('Failed to link deal to contact:', res.status, errorText);
      }
    } catch (error) {
  ...
  const handleRemoveDeal = async (deal: Deal) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errorText = await res.text();
        console.error('Failed to unlink deal from contact:', res.status, errorText);
      }
    } catch (error) {
